...
def load_model(model_path, state_size, action_size):
    model = PPO(state_size, action_size)
    model.load_state_dict(torch.load(model_path))
    model.eval()  # Set the model to evaluation mode
    return model

def test_model_on_new_route(model, map_details, route_file):
    # Create the environment with the new route file
    for map_ in map_details:
        if map_['route_file'] == route_file:
            env = sumo_rl.env(
                net_file=map_['net_file'],
                route_file=route_file,  # New route file for testing
                use_gui=False,
                num_seconds=2000,
                reward_fn="weighted",
                fixed_ts=False
    ...
    # Set up agents (make sure to load the model)
    ppo_agents = {
        ts: PPOAgent(
    ...
    for agent in env.agent_iter():
        ...
        # Use the trained model to act (no training here)
        action = ppo_agents[agent].act(s) if not done else None

        # Take a step in the environment with the action
        env.step(action)

        # Optionally, log results or monitor performance
        print(f"Agent: {agent}, Step: {env.step_counter}, Reward: {r}")

    env.close()

# Example Usage:
if __name__ == "__main__":
    model_path = "ppo_model_run_1.pth"  # Path to the saved model
    route_file = "new_route_file.rou.xml"  # New route file for testing
    
    # You must know the state and action space dimensions before loading
    state_size = 10  # Replace with your actual state size
    action_size = 5  # Replace with your actual action space size

    # Load the pre-trained PPO model
    model = load_model(model_path, state_size, action_size)

    # Test the model on the new route
    test_model_on_new_route(model, map_details, route_file)
